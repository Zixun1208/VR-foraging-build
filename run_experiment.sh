...
# New iteration/trial structure
TOTAL_ITERATIONS=1            # Number of (training+probing) iterations
TRAINING_TRIALS_PER_ITER=1    # Number of training trials per iteration
PROBING_TRIALS_PER_ITER=1     # Number of probing trials per iteration
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        # New arguments
        --total-iterations) TOTAL_ITERATIONS="$2"; shift ;;
        --training-trials) TRAINING_TRIALS_PER_ITER="$2"; shift ;;
        --probing-trials) PROBING_TRIALS_PER_ITER="$2"; shift ;;
...
# ----------------- MAIN ITERATION LOOP -----------------
echo "Running $TOTAL_ITERATIONS iteration(s) with $TRAINING_TRIALS_PER_ITER training trial(s) and $PROBING_TRIALS_PER_ITER probing trial(s) per iteration." | tee -a "$SCRIPT_LOG"

for ((iter=1; iter<=TOTAL_ITERATIONS; iter++)); do
    echo "==== Starting iteration $iter of $TOTAL_ITERATIONS ====" | tee -a "$SCRIPT_LOG"

    # -------- TRAINING TRIALS IN THIS ITERATION --------
    for ((t=1; t<=TRAINING_TRIALS_PER_ITER; t++)); do
        if (( TRAINING_SESSION_TIME > 0 )); then
            echo "Starting TRAINING trial $t of $TRAINING_TRIALS_PER_ITER in iteration $iter" | tee -a "$SCRIPT_LOG"
            activate_conda
            ...
            echo "Running con_led.py for TRAINING..." | tee -a "$SCRIPT_LOG"
            python3 "$con_led_exe" --zones "0:0,1:0" >> "$CON_LED_LOG" 2>&1 &
            PIDS+=($!)
    ...
    for ((p=1; p<=PROBING_TRIALS_PER_ITER; p++)); do
        if (( PROBING_SESSION_TIME > 0 )); then
            echo "Starting PROBING trial $p of $PROBING_TRIALS_PER_ITER in iteration $iter" | tee -a "$SCRIPT_LOG"
            activate_conda
            ...
            echo "Running con_led.py for PROBING..." | tee -a "$SCRIPT_LOG"
            python3 "$con_led_exe" --zones "0:60,1:120" >> "$CON_LED_LOG" 2>&1 &
            PIDS+=($!)
    ...
    echo "==== Finished iteration $iter of $TOTAL_ITERATIONS ====" | tee -a "$SCRIPT_LOG"
done
